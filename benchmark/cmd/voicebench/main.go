...
func cmdCompare(args []string) error {
	...
	for i, dir := range dirs {
		...
		cfg.Runs = append(cfg.Runs, report.LabeledRun{Label: label, Dir: dir, Summary: sum})
	}
...
func cmdSTT(args []string) error {
	...
	for i, line := range splitLines(string(raw)) {
		...
		if hypothesis == "" {
			hypothesis = rec.Transcript
		}
		rawAlign := score.ScoreWER(reference, hypothesis, false)
		normAlign := score.ScoreWER(reference, hypothesis, true)
		n++
