...
// comparePointRow renders one millisecond or turn-count metric, where less is better.
func comparePointRow(name string, stats []runStats, pick func(runStats) float64) compareRow {
	row := compareRow{Name: name, Best: 0, Star: make([]bool, len(stats))}
	for i, st := range stats {
		v := pick(st)
		row.Cells = append(row.Cells, compareCell{Text: fmt.Sprintf("%.0f", v), Value: v})
		if i > 0 && v < row.Cells[row.Best].Value {
			row.Best = i
		}
	}
	return row
