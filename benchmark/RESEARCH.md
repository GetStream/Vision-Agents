...
Which acceleration configuration counts as "as shipped" is an open decision. [`customer_support.py`](../examples/agents/customer_support/customer_support.py) is the only concrete triple in the repo. Whatever is picked becomes the number the trend line and the competitor table quote, so it should be pinned in the manifest and not changed without a methodology bump.
...
- Which acceleration configuration is "as shipped" for the headline row. `customer_support.py`'s Gemini-plus-Inworld triple is the only concrete example. The pick is quoted forever, until a methodology bump.
