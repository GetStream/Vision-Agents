...
Every current example uses a third style neither target exercises. [`examples/voice_agents/customer_support/customer_support.py`](../examples/voice_agents/customer_support/customer_support.py) is `Agent(config="customer_support", llm=acceleration.Accelerated(stt=..., tts=..., model=..., subagent=...))` after `sync_agent(...)`. [`examples/voice_agents/restaurant_orders/restaurant_orders.py`](../examples/voice_agents/restaurant_orders/restaurant_orders.py) is `Agent(config="restaurant_orders")` behind `StreamDispatch`. `Agent(config=...)` fills in edge, llm, agent_user, and phone from the stored config via `_accelerated_defaults`. Instructions, skills, and knowledge come from the agent directory, not from a prompt string.
