...
_KEYTERMS_PATH = Path(__file__).resolve().parent.parent / "contracts" / "keyterms.json"


def pack_keyterms(pack: str) -> list[str]:
    """Return the shared pack keyterm list both targets send to the transcriber."""
    packs = json.loads(_KEYTERMS_PATH.read_text())
    return list(packs.get(pack, []))
