"""Streaming Breeze TTS 2 text-to-speech over a Baseten WebSocket.

Wire protocol, the same one the S2 Pro deployment next door speaks, so the Go providers
differ only in what they put in a flush:
...
The model is served by the reference streaming API from the breeze-tts repository, started
once per replica and reached over localhost. That API takes a whole utterance as multipart
form data, which is why deltas accumulate here and generation begins on flush.

Unlike the engine next door it serves one request at a time, so config.yaml pins
predict_concurrency to 1 and a busy deployment scales by adding replicas.
"""
...
MODEL_CACHE_DIR = "/app/model_cache/breeze-tts-2"

# An English vocal event, which Breeze writes in parentheses. The agent writes every
# direction in square brackets, because parentheses are ordinary punctuation that a reply
# may use for something else, so they are translated here rather than upstream.
#
# The character class is deliberately ASCII: a Chinese event is already written in square
# brackets, such as [笑], and must pass through untouched.
ENGLISH_DIRECTION = re.compile(r"\[([A-Za-z][A-Za-z '\u2019-]{0,38})\]")
...
def as_vocal_events(text: str) -> str:
    """Rewrites [sigh] into (sigh), which is the syntax this model performs."""
    return ENGLISH_DIRECTION.sub(r"(\1)", text)


class Model:
    def __init__(self, lazy_data_resolver, **kwargs) -> None:
        self._lazy_data_resolver = lazy_data_resolver
        self._port = int(os.getenv("ENGINE_PORT", "7860"))
        self._startup_s = float(os.getenv("ENGINE_STARTUP_S", "900"))
        self._sample_rate = int(os.getenv("SAMPLE_RATE", "24000"))
        self._fast = os.getenv("ENGINE_FAST", "1") == "1"
        self._cfg_scale = float(os.getenv("INSTRUCTION_CFG_SCALE", "4"))
    ...
    def _start_engine(self) -> subprocess.Popen:
        ...
        return subprocess.Popen(command, cwd="/opt/breeze-tts")
    ...
    async def _speak(
        ...
        form: dict[str, str] = {"text": as_vocal_events(text)}

        # An instruction without reference audio designs a voice from the description; with
        # it, the reference decides who is speaking and the instruction how.
        instruction = frame.get("instruction") or ""
        if instruction:
            form["instruction"] = instruction
            form["cfg_scale"] = str(self._cfg_scale)
        if frame.get("reference_text"):
            form["ref_text"] = frame["reference_text"]

        files = {}
        reference = frame.get("reference_audio")
        if reference:
            files["ref_audio"] = ("reference.wav", open(reference, "rb"), "audio/wav")
        ...
        try:
            async with self._client.stream(
                "POST", "/v1/audio/speech", data=form, files=files or None
            ) as response:
                if response.status_code != 200:
