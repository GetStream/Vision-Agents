...
// TogetherNemotronSocketSuite exercises the provider against a server that is answering,
// which is where the handshake, the credentials, the audio encoding and the wait for the
// tail of a call live.
type TogetherNemotronSocketSuite struct {
	suite.Suite
}

func TestTogetherNemotronSocketSuite(t *testing.T) {
	suite.Run(t, new(TogetherNemotronSocketSuite))
}
...
func (s *TogetherNemotronSocketSuite) connect(fake *fakeRealtime, options Options) (*STT, *websocket.Conn) {
...
func (s *TogetherNemotronSocketSuite) build(fake *fakeRealtime, options Options) *STT {
...
func (s *TogetherNemotronSocketSuite) ctx() context.Context {
...
func (s *TogetherNemotronSocketSuite) speak(provider *STT, samples []int16) {
...
func (s *TogetherNemotronSocketSuite) nextFrame(conn *websocket.Conn) clientMessage {
...
func (s *TogetherNemotronSocketSuite) TestTheSessionAsksToTranscribeWithTheEnglishModel() {
...
func (s *TogetherNemotronSocketSuite) TestTheSessionOpensOnTheMultilingualModelWhenAskedForIt() {
	// A multilingual call that opened on the English model would look like a bad
	// transcript rather than a misconfigured session.
	fake := newFakeRealtime()
	defer fake.close()
	provider, _ := s.connect(fake, Options{Model: MultilingualModel})
	defer func() { _ = provider.Close() }()

	s.Equal(MultilingualModel, (<-fake.requests).URL.Query().Get("model"))
}

func (s *TogetherNemotronSocketSuite) TestTheKeyAndTheProtocolVersionGoInTheHeaders() {
	fake := newFakeRealtime()
	defer fake.close()
...
func (s *TogetherNemotronSocketSuite) TestAudioArrivesBase64EncodedInsideAFrame() {
	fake := newFakeRealtime()
	defer fake.close()
...
func (s *TogetherNemotronSocketSuite) TestStartWaitsForTheSessionToOpen() {
	fake := newFakeRealtime()
...
func (s *TogetherNemotronSocketSuite) TestStartReportsAHandshakeTheServerRejected() {
	fake := newFakeRealtime()
...
func (s *TogetherNemotronSocketSuite) TestClosingCommitsTheAudioTheServerHadNotTranscribed() {
	fake := newFakeRealtime()
	defer fake.close()
...
func (s *TogetherNemotronSocketSuite) TestClosingASettledCallDoesNotWaitOutTheTimeout() {
...
func (s *TogetherNemotronSocketSuite) TestClosingGivesUpOnAServerThatNeverAnswers() {
	fake := newFakeRealtime()
	defer fake.close()
...
func (s *TogetherNemotronSocketSuite) nextFinal(provider *STT) string {
...
func (s *TogetherNemotronSocketSuite) finalText(provider *STT) string {
	var settled string
