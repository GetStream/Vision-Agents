...
func TestS2ProIntegrationSuite(t *testing.T) {
	suite.Run(t, new(S2ProIntegrationSuite))
}

func (s *S2ProIntegrationSuite) SetupSuite() {
	if os.Getenv("S2PRO_WS_URL") == "" {
		s.T().Skip("S2PRO_WS_URL not set")
	}
...
func (s *S2ProIntegrationSuite) start() *TTS {
...
func (s *S2ProIntegrationSuite) collect(provider *TTS, until func(tts.Event) bool) []tts.Event {
...
func (s *S2ProIntegrationSuite) TestSynthesizesRealSpeech() {
	provider := s.start()
	defer func() {
...
func (s *S2ProIntegrationSuite) TestDeltasStreamUpstreamAndFlushIntoOneUtterance() {
	provider := s.start()
	defer func() {
		s.Require().NoError(provider.Close())
	}()
...
func (s *S2ProIntegrationSuite) TestInterruptStopsALongUtterance() {
	provider := s.start()
	defer func() {
		s.Require().NoError(provider.Close())
	}()
