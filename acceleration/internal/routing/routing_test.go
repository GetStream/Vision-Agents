...
func (s *RoutingSuite) TestDefaultConfigDeclaresVisionOnOpenAIGeminiAnthropicAndXAI() {
	config, err := DefaultConfig()
	s.Require().NoError(err)

	for _, provider := range config[LLM].Providers {
		if provider.Provider == "openai" || provider.Provider == "gemini" || provider.Provider == "anthropic" || provider.Provider == "xai" {
