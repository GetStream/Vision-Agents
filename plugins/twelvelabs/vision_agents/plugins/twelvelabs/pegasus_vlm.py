...
class PegasusVLM(llm.VideoLLM):
    ...
    def __init__(
        ...
        # Buffer enough frames to cover the requested clip window.
        self._max_frames = max(1, int(fps * clip_seconds))
    ...
    async def simple_response(
        ...
        try:
            ...
            logger.info(f"Pegasus response: {total_text}")
    ...
    def _encode_clip(self, frames: list[av.VideoFrame]) -> bytes:
        ...
        try:
            stream = container.add_stream("libx264", rate=int(self.fps) or 1)
            stream.width = width
