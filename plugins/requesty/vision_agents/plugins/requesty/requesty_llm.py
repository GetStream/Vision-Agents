"""Requesty LLM implementation using Chat Completions API.

Requesty is an OpenAI-compatible LLM gateway/router that exposes many models
from different providers through a single endpoint. This implementation uses
the Chat Completions API for all models as it's the industry standard and
...
PLUGIN_NAME = "requesty"


class RequestyLLM(LLM):
    """Requesty LLM using Chat Completions API for all models.

    Requesty-specific handling:
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize Requesty LLM.

        Args:
            api_key: Requesty API key. Defaults to REQUESTY_API_KEY env var.
            base_url: Requesty API base URL.
            model: Model to use (e.g., 'openai/gpt-4o-mini', 'anthropic/claude-sonnet-4-5').
