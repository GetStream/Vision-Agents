    ...
    async def _create_chunks(
        ...
        for i, chunk_text in enumerate(chunks):
            ...
            try:
                ...
                logger.error(f"Failed to create chunk {i} for message {message.id}: {e}")
                raise
        
        # Store first chunk ID for backward compatibility
        if state.backend_message_ids:
            self.internal_ids_to_stream_ids[message.id] = state.backend_message_ids[0]

    async def _update_chunks(
        self, message: Message, state: MessageState, new_chunks: List[str], completed: bool
    ):
        ...
        for i in range(min(old_chunk_count, new_chunk_count)):
            ...
            try:
                if completed:
                    ...
                    logger.debug(f"Finalized chunk {i+1}/{new_chunk_count} (ID: {chunk_id})")
                else:
                    ...
                    logger.debug(f"Updated chunk {i+1}/{new_chunk_count} (ID: {chunk_id})")
            except Exception as e:
        ...
        if new_chunk_count > old_chunk_count:
            for i in range(old_chunk_count, new_chunk_count):
                ...
                try:
                    ...
                    chunk_id = response.data.message.id
                    state.backend_message_ids.append(chunk_id)
                    logger.debug(f"Created new chunk {i+1}/{new_chunk_count} (ID: {chunk_id})")
                except Exception as e:
