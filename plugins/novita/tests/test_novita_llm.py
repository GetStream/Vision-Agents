"""Tests for Novita AI LLM plugin."""
...
from vision_agents.plugins.novita import LLM
...
class TestNovitaLLM:
    ...
    async def test_strict_mode_for_tools(self):
        """Tools with required params should have strict mode enabled."""
    ...
    async def test_no_strict_mode_without_required(self):
        """Tools without required params should not have strict mode."""
        llm = LLM(model="moonshotai/kimi-k2.5")
        tools = [
            {
                "name": "test_tool",
    ...
    async def llm(self) -> LLM:
        """Fixture for Novita LLM with conversation."""
    ...
    async def test_function_calling(self):
        """Test function calling with Novita AI."""
        skip_without_api_key()
        llm = LLM(model="moonshotai/kimi-k2.5")
