#!/usr/bin/env python3
"""
Example: Text-to-Speech with Palabra AI using the Agent class

This minimal example shows how to:
1. Create an Agent with Deepgram STT, a Gemini LLM and Palabra TTS
2. Join a Stream video call
3. Greet users and respond to spoken input

Palabra TTS is a streaming plugin: the agent forwards each finished sentence of
the LLM response over an already-open WebSocket, so the bot starts speaking
...
async def create_agent(**kwargs) -> Agent:
    """Create an agent with Deepgram STT, a Gemini LLM and Palabra TTS."""
    agent = Agent(
        edge=getstream.Edge(),
        ...
        stt=deepgram.STT(),
        llm=gemini.LLM(),
        tts=palabra.TTS(voice_id="default_low", language="en"),
    )
...
async def join_call(agent: Agent, call_type: str, call_id: str, **kwargs) -> None:
    ...
    logger.info("Starting Palabra TTS voice bot")
