"""OrcaRouter LLM implementation using Chat Completions API.

OrcaRouter routes to many models from different providers behind one
OpenAI-compatible endpoint. This implementation uses Chat Completions API for
all models, which is the format OrcaRouter serves on ``/v1/chat/completions``.
"""
...
PLUGIN_NAME = "orcarouter"

# Models that reliably support tool calling via Chat Completions API.
# Used as fallbacks when orcarouter/auto routes to a model without tool support.
...
class OrcaRouterLLM(LLM):
    """OrcaRouter LLM using Chat Completions API for all models.

    OrcaRouter-specific handling:
    ...
    - Model ids carry a vendor namespace (``openai/gpt-5.5``), so the namespace
      is kept as given rather than rewritten
    """
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize OrcaRouter LLM.

        Args:
            api_key: OrcaRouter API key. Defaults to ORCAROUTER_API_KEY env var.
            base_url: OrcaRouter API base URL.
            model: Model to use (e.g. 'openai/gpt-4o-mini', 'anthropic/claude-opus-4.8').
                ``orcarouter/auto`` selects an upstream per request; it is not the
                default because its candidate pool can include models without tool
                calling.
    ...
    def _is_auto_model(self, model: Optional[str] = None) -> bool:
        ...
        return model in ("orcarouter/auto",)
    ...
    async def _create_response_internal(
        ...
        if tools:
            ...
            # orcarouter/auto may route to models that don't support tools.
            # Add fallbacks to ensure tool calls work.
            if self._is_auto_model(effective_model):
                logger.info(
                    "orcarouter/auto with tools: adding fallbacks %s",
                    TOOL_SUPPORTING_MODELS,
                )
                request_kwargs["extra_body"] = self._tool_fallback_body()
    ...
    async def _process_streaming_response(
        ...
        async for chunk in cast(AsyncStream[ChatCompletionChunk], response):
            ...
            if finish_reason:
                ...
                # Upstreams behind the router can emit multiple
                # `finish_reason="tool_calls"` chunks; finalize only when there's
                # still pending data to consume, otherwise the second pass
                # overwrites the result with [].
