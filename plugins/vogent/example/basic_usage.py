...
async def example_with_real_audio():
    ...
    conversation = InMemoryConversation(
        instructions="You are a helpful assistant", messages=[]
    )

    # Add a previous message for context
    await conversation.send_message(
        role="assistant", user_id="assistant", content="What is your phone number"
    )

    # Convert to PcmData
    pcm_data = PcmData(samples=audio_samples, format="f32", sample_rate=sample_rate)
