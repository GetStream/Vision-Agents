...
4. **OpenAI Account**: Set an `OPENAI_API_KEY` for the example LLM.
...
3. **Set up environment variables:**
   Copy `.env.example` to `.env` and fill in `STREAM_API_KEY`, `STREAM_API_SECRET`, `SPEECHIFY_API_KEY`, `DEEPGRAM_API_KEY`, and `OPENAI_API_KEY`.
