"""Inbound Telnyx phone example running the whole pipeline on Telnyx.

Unlike `inbound_call.py`, which bridges the call into a realtime model, this
example uses Telnyx for transport, speech to text, inference, and text to
speech. The only non-Telnyx pieces are Stream for the call and a turn detector,
which Telnyx's transcription endpoint does not provide.

Run after starting ngrok and routing your Telnyx number to a Call Control App:

    NGROK_URL=example.ngrok-free.app uv run plugins/telnyx/examples/voice_agent_call.py
"""
...
async def create_agent() -> tuple[Agent, StreamEdge]:
    ...
    agent = Agent(
        ...
        # 8000 is what TelnyxMediaStream decodes PCMU telephony audio to, so
        # the transcriber receives it without an upsample that adds nothing.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run an inbound Telnyx call with a Telnyx STT/LLM/TTS pipeline."
    )
