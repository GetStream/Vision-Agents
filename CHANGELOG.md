...
Two comparisons disagreed with themselves. `end_state` compared with `fmt.Sprint` while `expected_tools` went through `MatchStructuredValue`, so an agent that wrote `Peanut` on the ticket satisfied one gate and failed the other; both now use the same comparator. And the restaurant contract asked for "one moment, checking" *while tools run*, which models wrote at the end of the reply — measured on `tool_filler`, the filler arrived 2636 ms after a 3000 ms lookup had already returned. The filler now leads any turn that calls a tool.
