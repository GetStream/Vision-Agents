...
class DotaGameProcessor(BaseProcessor):
    ...
    def setup_client(self) -> bool:
        ...
        try:
            ...
            if self.api_token:
                self.stratz_api = stratz.Api(
                    lang=code_English, api_token=self.api_token
                )
                self.logger.info("Stratz API client configured with API token")
            else:
                self.stratz_api = stratz.Api(lang=code_English)
                self.logger.info(
                    "Stratz API client configured without API token (public access)"
                )
    ...
    def get_live_matches(self) -> Optional[List[Dict[str, Any]]]:
        ...
        if matches_data is None:
            self.logger.info(
                "Stratz client approach failed, trying GraphQL HTTP fallback..."
            )
    ...
    def _get_live_matches_via_graphql(self) -> Optional[List[Dict[str, Any]]]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if (
                    "data" in data
                    and "live" in data["data"]
                    and "matches" in data["data"]["live"]
                ):
    ...
    def _convert_stratz_match_to_dict(self, match) -> Dict[str, Any]:
        ...
        match_dict = {
            ...
            "radiant_score": safe_get(match, "radiantScore")
            or safe_get(match, "radiant_score"),
        ...
        if radiant_team:
            match_dict["radiant_team"] = {
                "team_id": safe_get(radiant_team, "id")
                or safe_get(radiant_team, "team_id"),
        ...
        if players:
            ...
            for player in players:
                player_dict = {
                    "account_id": safe_get(player, "steamAccountId")
                    or safe_get(player, "account_id"),
                    "hero_id": safe_get(player, "heroId")
                    or safe_get(player, "hero_id"),
                    "kills": safe_get(player, "kills"),
                    "deaths": safe_get(player, "deaths"),
                    "assists": safe_get(player, "assists"),
                    "net_worth": safe_get(player, "netWorth")
                    or safe_get(player, "net_worth"),
                    "level": safe_get(player, "level"),
                    "is_radiant": safe_get(player, "isRadiant")
                    or safe_get(player, "is_radiant"),
                }
    ...
    def _get_match_via_stratz(self, match_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.debug(
                f"Stratz client approach failed for match {match_id}: {e}"
            )
            return None
    ...
    def _get_match_via_graphql_http(self, match_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            payload = {"query": query, "variables": {"matchId": match_id}}
            ...
            if response.status_code == 200:
                ...
                if "data" in data and "match" in data["data"] and data["data"]["match"]:
                    ...
                    self.logger.error(
                        f"Match {match_id} not found or unexpected GraphQL response structure"
                    )
                    return None
            else:
                self.logger.error(
                    f"GraphQL request failed for match {match_id} with status: {response.status_code}"
                )
                return None
    ...
    def _convert_stratz_detailed_match_to_dict(self, match) -> Dict[str, Any]:
        ...
        match_dict = {
            ...
            "game_mode": safe_get(match, "gameMode") or safe_get(match, "game_mode"),
            "lobby_type": safe_get(match, "lobbyType") or safe_get(match, "lobby_type"),
            "duration": safe_get(match, "durationSeconds")
            or safe_get(match, "duration"),
            "start_time": safe_get(match, "startDateTime")
            or safe_get(match, "start_time"),
            "radiant_score": safe_get(match, "radiantKills")
            or safe_get(match, "radiant_score"),
        ...
        if players:
            ...
            for player in players:
                player_dict = {
                    ...
                    "deaths": safe_get(player, "deaths"),
                    "assists": safe_get(player, "assists"),
                    "last_hits": safe_get(player, "lastHits")
                    or safe_get(player, "last_hits"),
                    "denies": safe_get(player, "denies"),
                    "gold_per_min": safe_get(player, "goldPerMinute")
                    or safe_get(player, "gold_per_min"),
                    "xp_per_min": safe_get(player, "experiencePerMinute")
                    or safe_get(player, "xp_per_min"),
                    "level": safe_get(player, "level"),
                    "net_worth": safe_get(player, "netWorth")
                    or safe_get(player, "net_worth"),
                    "is_radiant": safe_get(player, "isRadiant")
