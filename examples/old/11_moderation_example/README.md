...
```bash
uv sync --directory examples/old/11_moderation_example
```

2. Create a `.env` file with your API keys:

```bash
cd examples/old/11_moderation_example
cp env.example .env
...
```bash
uv run --directory examples/old/11_moderation_example moderation_example.py run
```
