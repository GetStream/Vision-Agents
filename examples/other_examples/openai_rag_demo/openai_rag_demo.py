"""
OpenAI RAG Demo - Retrieval-Augmented Generation with OpenAI Vector Store

This example demonstrates how to use OpenAI's Vector Store API for RAG.
The demo creates a knowledge base from sample documents and shows how to:
1. Upload files to OpenAI's Vector Store
...
from vision_agents.plugins.openai import LLM, OpenAIVectorStoreRAG
...
async def create_sample_files() -> list[str]:
    ...
    temp_dir = tempfile.mkdtemp(prefix="openai_rag_demo_")
    file_paths = []
...
async def main():
    print("\n" + "=" * 60)
    print("🔍 OpenAI RAG Demo - Vector Store with Retrieval-Augmented Generation")
    print("=" * 60 + "\n")

    # Create the RAG provider
    print("📦 Initializing OpenAI Vector Store RAG provider...")
    rag = OpenAIVectorStoreRAG(store_name="openai-rag-demo-store")
    ...
    print("\n☁️  Uploading documents to OpenAI Vector Store...")
    ...
    print(f"\n✅ Vector Store created: {rag.store_id}")

    # Create LLM and attach RAG provider
    print("\n🤖 Initializing OpenAI LLM with RAG...")
    ...
    for query in queries:
        ...
        # Handle response - it may be a string or an object with .text
        response_text = response.text if hasattr(response, "text") else str(response)
        print(
            f"💡 Answer: {response_text[:500]}{'...' if len(response_text) > 500 else ''}"
        )
