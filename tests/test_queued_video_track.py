...
class TestQueuedVideoTrack(BaseTest):
    ...
    async def test_queued_video_track_returns_last_frame_when_empty(self):
        ...
        assert mean_green2 > mean_red2, "Second frame should still be predominantly green (last frame)"
    ...
    async def test_queued_video_track_fps_timing(self):
        ...
        track = QueuedVideoTrack(width=320, height=240, fps=10)  # 10 FPS = 0.1s per frame
