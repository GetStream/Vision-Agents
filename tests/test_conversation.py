...
pytestmark = pytest.mark.skip(
    reason="StreamConversation class removed - tests need migration to new architecture"
)
...
class TestInMemoryConversation:
    ...
    def test_add_message(self, conversation):
        ...
        new_message = Message(
            original=None, content="New message", role="user", user_id="user2"
        )
    ...
    def test_add_message_with_completed(self, conversation):
        ...
        new_message1 = Message(
            original=None, content="Generating message", role="user", user_id="user2"
        )
        ...
        new_message2 = Message(
            original=None, content="Complete message", role="user", user_id="user3"
        )
    ...
    def test_streaming_message_handle(self, conversation):
        ...
        handle = conversation.start_streaming_message(
            role="assistant", initial_content="Hello"
        )
    ...
    def test_multiple_streaming_handles(self, conversation):
        ...
        handle1 = conversation.start_streaming_message(
            role="user", user_id="user1", initial_content="Question: "
        )
        handle2 = conversation.start_streaming_message(
            role="assistant", initial_content="Answer: "
        )
        ...
        msg1 = next(
            msg for msg in conversation.messages if msg.id == handle1.message_id
        )
        msg2 = next(
            msg for msg in conversation.messages if msg.id == handle2.message_id
        )
...
class TestStreamConversation:
    ...
    def stream_conversation(self, mock_chat_client, mock_channel):
        ...
        conversation.internal_ids_to_stream_ids = {"msg-0": "stream-msg-0"}
    ...
    def test_add_message(self, stream_conversation, mock_chat_client):
        ...
        new_message = Message(
            original=None, content="Test message", role="user", user_id="user123"
        )
        ...
        assert (
            stream_conversation.internal_ids_to_stream_ids["new-msg-id"]
            == "stream-message-123"
        )
        ...
        assert (
            update_args[1]["set"]["generating"] is False
        )  # completed=True means not generating

    def test_add_message_with_completed_false(
        self, stream_conversation, mock_chat_client
    ):
        ...
        assert (
            update_args[1]["set"]["generating"] is True
        )  # completed=False means still generating
    ...
    def test_update_message_existing(self, stream_conversation, mock_chat_client):
        ...
        assert (
            call_args[1]["set"]["generating"] is True
        )  # not completed = still generating
    ...
    def test_update_message_completed_vs_generating(
        self, stream_conversation, mock_chat_client
    ):
    ...
    def test_update_message_no_stream_id(self, stream_conversation, mock_chat_client):
        ...
        new_msg = Message(original=None, content="Test", role="user", user_id="user3")
    ...
    def test_streaming_message_handle(self, stream_conversation, mock_chat_client):
        ...
        # Start a streaming message
        handle = stream_conversation.start_streaming_message(
            role="assistant", initial_content="Processing"
        )
    ...
    def test_multiple_streaming_handles(self, stream_conversation, mock_chat_client):
        ...
        handle1 = stream_conversation.start_streaming_message(
            role="user", user_id="user123", initial_content=""
        )
        handle2 = stream_conversation.start_streaming_message(
            role="assistant", initial_content=""
        )
        ...
        # Find messages by their handles to verify correct updates
        msg1 = next(
            msg for msg in stream_conversation.messages if msg.id == handle1.message_id
        )
        msg2 = next(
            msg for msg in stream_conversation.messages if msg.id == handle2.message_id
        )
        ...
        assert (
            mock_chat_client.ephemeral_message_update.call_count >= 4
        )  # 2 initial + 2 appends
    ...
    def test_worker_thread_async_operations(
        self, stream_conversation, mock_chat_client
    ):
        ...
        for i in range(5):
            msg = Message(
                original=None, content=f"Message {i}", role="user", user_id=f"user{i}"
            )
            messages.append(msg)
    ...
    def test_wait_for_pending_operations_timeout(
        self, stream_conversation, mock_chat_client
    ):
        ...
        # Add a message
        msg = Message(
            original=None, content="Slow message", role="user", user_id="user1"
        )
...
def test_stream_conversation_integration():
    ...
    channel = client.chat.get_or_create_channel(
        "messaging", str(uuid.uuid4()), data=ChannelInput(created_by_id=user.id)
    ).data.channel
    ...
    # Add a message
    message = Message(
        original=None, content="Hello from test", role="user", user_id=user.id
    )
    ...
    channel_data = client.chat.get_or_create_channel(
        "messaging", channel.id, state=True
    ).data
    ...
    handle = conversation.start_streaming_message(
        role="assistant", initial_content="Thinking"
    )
    assert conversation.wait_for_pending_operations(timeout=5.0)
    time.sleep(0.2)  # Give extra time for update operation
