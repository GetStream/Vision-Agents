...
class TestAudioQueue(BaseTest):
    ...
    async def test_audio_queue_put_and_get(self):
        ...
        pcm = PcmData(samples=samples, sample_rate=16000, format=AudioFormat.S16, channels=1)
    ...
    async def test_audio_queue_sample_rate_mismatch_warning(self, caplog):
        ...
        pcm1 = PcmData(samples=samples1, sample_rate=16000, format=AudioFormat.S16, channels=1)
        await queue.put(pcm1)
        
        # Then add 48kHz audio (should warn)
        samples2 = np.array([4, 5, 6], dtype=np.int16)
        pcm2 = PcmData(samples=samples2, sample_rate=48000, format=AudioFormat.S16, channels=1)
    ...
    async def test_audio_queue_different_sample_rates(self):
        ...
        pcm1 = PcmData(samples=samples1, sample_rate=8000, format=AudioFormat.S16, channels=1)
        await queue.put(pcm1)
    ...
    async def test_audio_queue_large_buffer(self):
        ...
        pcm = PcmData(samples=samples, sample_rate=48000, format=AudioFormat.S16, channels=1)
        await queue.put(pcm)
    ...
    async def test_audio_queue_preserve_data_integrity(self):
        ...
        original_samples = np.array([100, 200, 300, 400, 500, 600, 700, 800], dtype=np.int16)
        pcm = PcmData(samples=original_samples, sample_rate=16000, format=AudioFormat.S16, channels=1)
        await queue.put(pcm)
