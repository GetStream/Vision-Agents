...
class TestTTSEvents:
    ...
    def test_tts_error_event(self):
        ...
        event = TTSErrorEvent(
            error=error, error_code="TTS_001", context="synthesis", is_recoverable=True
        )
...
class TestVADEvents:
    ...
    def test_vad_speech_start_event(self):
        ...
        event = VADSpeechStartEvent(
            speech_probability=0.8, activation_threshold=0.5, frame_count=10
        )
    ...
    def test_vad_partial_event(self):
        ...
        event = VADPartialEvent(
            speech_probability=0.7, frame_count=25, is_speech_active=True
        )
...
class TestEventEnums:
    ...
    def test_event_types(self):
        ...
        for expected_type in expected_types:
            assert hasattr(EventType, expected_type.upper().replace("-", "_")), (
                f"Missing event type: {expected_type}"
            )
...
class TestEventEdgeCases:
    ...
    def test_event_with_none_values(self):
        ...
        event = STTTranscriptEvent(
            text="test", confidence=None, language=None, processing_time_ms=None
        )
        assert event.confidence is None
        assert event.language is None
        assert event.processing_time_ms is None
    ...
    def test_event_with_zero_values(self):
        ...
        event = STTTranscriptEvent(
            text="test", confidence=0.0, processing_time_ms=0.0, audio_duration_ms=0.0
        )
    ...
    def test_event_serialization_compatibility(self):
        ...
        event = STTTranscriptEvent(text="test", confidence=0.95, language="en")
...
class TestEventMetrics:
    ...
    def test_calculate_stt_metrics_mixed_data(self):
        ...
        events = [
            STTTranscriptEvent(
                text="Complete", confidence=0.9, processing_time_ms=100.0
            ),
    ...
    def test_calculate_tts_metrics_with_real_time_factors(self):
        ...
        assert (
            metrics["avg_real_time_factor"] == 1.0
        )  # (0.5+1.0+1.5)/3 (only 3 have values)
    ...
    def test_calculate_vad_metrics_with_durations_and_probabilities(self):
        ...
        events = [
            VADAudioEvent(
                audio_data=b"short", duration_ms=500.0, speech_probability=0.8
            ),
            VADAudioEvent(
                audio_data=b"medium", duration_ms=1000.0, speech_probability=0.9
            ),
            VADAudioEvent(
                audio_data=b"long", duration_ms=2000.0, speech_probability=0.95
            ),
        ]
        ...
        assert (
            pytest.approx(metrics["avg_speech_duration_ms"], 0.01) == 1166.67
        )  # (500+1000+2000)/3
        assert metrics["total_speech_duration_ms"] == 3500.0  # 500+1000+2000
        assert (
            pytest.approx(metrics["avg_speech_probability"], 0.001) == 0.883
        )  # (0.8+0.9+0.95)/3
    ...
    def test_calculate_vad_metrics_mixed_data(self):
        ...
        events = [
            VADAudioEvent(
                audio_data=b"complete", duration_ms=1000.0, speech_probability=0.9
            ),
        ...
        assert (
            pytest.approx(metrics["avg_speech_probability"], 0.001) == 0.85
        )  # (0.9+0.8)/2 = 1.7/2 = 0.85
    ...
    def test_calculate_metrics_with_realistic_event_sequence(self):
        ...
        events = [
            ...
            STTPartialTranscriptEvent(
                text="Hello wo", confidence=0.85, processing_time_ms=50.0
            ),
            STTTranscriptEvent(
                text="Hello world", confidence=0.92, processing_time_ms=120.0
            ),
            # TTS Events
            ...
            TTSSynthesisCompleteEvent(
                synthesis_id="synth1", synthesis_time_ms=200.0, real_time_factor=0.8
            ),
            # VAD Events
            VADAudioEvent(
                audio_data=b"speech1", duration_ms=800.0, speech_probability=0.9
            ),
            VADPartialEvent(speech_probability=0.85),
            VADAudioEvent(
                audio_data=b"speech2", duration_ms=1200.0, speech_probability=0.95
            ),
        ]
        ...
        assert (
            pytest.approx(stt_metrics["avg_confidence"], 0.001) == 0.907
        )  # (0.95+0.85+0.92)/3
        ...
        assert (
            pytest.approx(vad_metrics["avg_speech_duration_ms"], 0.01) == 1000.0
        )  # (800+1200)/2
        assert vad_metrics["total_speech_duration_ms"] == 2000.0  # 800+1200
        assert (
            pytest.approx(vad_metrics["avg_speech_probability"], 0.001) == 0.925
        )  # (0.9+0.95)/2
...
class TestEventSerialization:
    ...
    def test_serialize_event_with_error(self):
        ...
        event = STTErrorEvent(
            error=error, error_code="TEST_001", context="test context"
        )
    ...
    def test_serialize_event_with_audio_data(self):
        ...
        event = TTSAudioEvent(audio_data=audio_data, sample_rate=22050, channels=2)
    ...
    def test_deserialize_stt_transcript_event(self):
        ...
        original_event = STTTranscriptEvent(
            text="Hello world", confidence=0.95, language="en", processing_time_ms=100.0
        )
    ...
    def test_deserialize_event_with_audio_data_placeholder(self):
        ...
        original_event = TTSAudioEvent(
            audio_data=b"test audio", sample_rate=16000, channels=1
        )
    ...
    def test_deserialize_invalid_event_type(self):
        ...
        data = {"event_type": "invalid_event_type", "text": "Test text"}
    ...
    def test_json_round_trip_serialization(self):
        ...
        deserialized_events = [
            deserialize_event(event_data) for event_data in events_data
        ]
        ...
        assert (
            deserialized_events[2].audio_data is None
        )  # Should be removed during serialization
...
class TestEventFiltering:
    ...
    def test_event_filter_edge_cases(self):
        ...
        assert (
            filter_empty.matches(event) is True
        )  # Empty sets = no filtering = match everything
...
class TestEventRegistry:
    ...
    def test_event_registry_filtering(self):
        ...
        events = [
            STTTranscriptEvent(
                text="high_conf", confidence=0.95, session_id="session1"
            ),
...
class TestGlobalEventSystem:
    ...
    def test_global_registry_access(self):
        ...
        test_events = [
            e for e in global_events if hasattr(e, "text") and e.text == "global_test"
        ]
    ...
    def test_global_event_consistency(self):
        ...
        registry_test_events = [
            e
            for e in registry_events
            if hasattr(e, "text") and e.text == "consistency_test"
        ]
        logger_test_events = [
            e
            for e in logger_events
            if hasattr(e, "text") and e.text == "consistency_test"
        ]
