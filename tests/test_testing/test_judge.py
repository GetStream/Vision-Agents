...
class TestLLMJudge:
    ...
    async def test_malformed_json_raises_judge_error(self, transcript):
        with pytest.raises(JudgeError, match="Could not parse JSON"):
            await LLMJudge(ScriptedLLM("not json")).evaluate_conversation(
                transcript, [CONCISE]
            )

    async def test_missing_results_list_raises_judge_error(self, transcript):
        with pytest.raises(JudgeError, match="Missing 'results'"):
            await LLMJudge(ScriptedLLM('{"verdict": "pass"}')).evaluate_conversation(
                transcript, [CONCISE]
            )
    ...
    async def test_unknown_verdict_raises_judge_error(self, transcript):
        llm = ScriptedLLM(_results({"name": "concise", "verdict": "maybe"}))

        with pytest.raises(JudgeError, match="Unknown verdict 'maybe'"):
            await LLMJudge(llm).evaluate_conversation(transcript, [CONCISE])

    async def test_non_string_reason_raises_judge_error(self, transcript):
        llm = ScriptedLLM(
            _results({"name": "concise", "verdict": "pass", "reason": {"code": 1}})
        )
    ...
    async def test_empty_llm_response_raises_judge_error(self, transcript):
