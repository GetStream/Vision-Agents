...
class MCPBaseServer(ABC):
    ...
    async def _call_with_retry(self, operation_name: str, operation_func, *args, **kwargs):
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt < max_retries:
                    self.logger.warning(f"{operation_name} failed (attempt {attempt + 1}/{max_retries + 1}): {e}. Reconnecting...")
                    await self.disconnect()
    ...
    async def _call_tool_impl(self, name: str, arguments: Dict[str, Any]) -> types.CallToolResult:
    ...
    async def _get_prompt_impl(self, name: str, arguments: Dict[str, Any]) -> types.GetPromptResult:
    ...
    async def call_tool(self, name: str, arguments: Dict[str, Any]) -> types.CallToolResult:
        ...
        return await self._call_with_retry("call_tool", self._call_tool_impl, name, arguments)
    ...
    async def read_resource(self, uri: str) -> types.ReadResourceResult:
        ...
        return await self._call_with_retry("read_resource", self._read_resource_impl, uri)
    ...
    async def get_prompt(self, name: str, arguments: Dict[str, Any]) -> types.GetPromptResult:
        ...
        return await self._call_with_retry("get_prompt", self._get_prompt_impl, name, arguments)
