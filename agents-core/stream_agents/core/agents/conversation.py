...
class Conversation(ABC):
    ...
    async def start_streaming_message(self, role: str = "assistant", user_id: Optional[str] = None, 
        ...
        Example:
            # Simple usage
            handle = await conversation.start_streaming_message()
            await conversation.append_to_message(handle, "Processing...")
            await conversation.replace_message(handle, "Here's the answer: ")
            await conversation.append_to_message(handle, "42")
            await conversation.complete_message(handle)
            
            # Multiple concurrent streams
            user_handle = await conversation.start_streaming_message(role="user", user_id="user123")
            assistant_handle = await conversation.start_streaming_message(role="assistant")
            
            # Update both independently
            await conversation.append_to_message(user_handle, "Hello")
            await conversation.append_to_message(assistant_handle, "Hi there!")
            
            # Complete in any order
            await conversation.complete_message(user_handle)
            await conversation.complete_message(assistant_handle)
        """
        message = Message(
            original=None,
    ...
    async def replace_message(self, handle: StreamHandle, text: str, completed=False):
...
class StreamConversation(InMemoryConversation):
    ...
    async def add_message(self, message: Message, completed: bool = True):
        ...
        if completed:
            return await self.chat_client.update_message_partial(
                stream_id,
                user_id=message.user_id,
                set={"text": message.content, "generating": not completed},
            )
        else:
            return await self.chat_client.ephemeral_message_update(
                stream_id,
                user_id=message.user_id,
                set={"text": message.content, "generating": not completed},
            )
    ...
    async def update_message(self, message_id: str, input_text: str, user_id: str, replace_content: bool, completed: bool):
        ...
        await super().update_message(message_id, input_text, user_id, replace_content, completed)
