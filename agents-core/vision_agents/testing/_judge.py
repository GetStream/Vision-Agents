...
class Judge(Protocol):
    ...
    async def evaluate_conversation(
        ...
        *,
        ...
        Raises:
            JudgeError: If the judge itself fails rather than the conversation.
        """
        ...
...
class LLMJudge:
    ...
    async def evaluate_conversation(
        ...
        *,
        ...
        Raises:
            ...
            JudgeError: If the judge LLM fails or returns no usable verdict.
        """
        if not events:
    ...
    def _parse_verdict(cls, text: str, criteria: Sequence[Criterion]) -> JudgeVerdict:
        ...
        Raises:
            JudgeError: If the text is not a recognisable list of results.
        """
        try:
        ...
        if not isinstance(results, list):
            raise JudgeError(
                f"Missing 'results' list in LLM response: {text[:_RESPONSE_PREVIEW_MAX_LEN]}"
            )
    ...
    def _parse_criterion(name: str, raw: dict[str, object]) -> CriterionVerdict:
        ...
        if not isinstance(verdict, str) or not isinstance(reason, str):
            raise JudgeError(f"Malformed verdict for criterion {name!r}: {raw!r}")
        verdict = verdict.lower()
        if verdict not in ("pass", "fail"):
            raise JudgeError(f"Unknown verdict '{verdict}' for criterion {name!r}.")
